...
from nthx_fusion.face_creator import get_one_face, get_static_faces
from nthx_fusion.face_tracker import track_faces
from nthx_fusion.types import Face, FaceSelectorOrder, Gender, Race, Score, VisionFrame
...
def sort_and_filter_faces(source_faces : List[Face], target_faces : List[Face]) -> List[Face]:
	if target_faces:
		...
		if face_selector_gender in nthx_fusion.choices.genders:
			target_faces = filter_faces_by_gender(target_faces, face_selector_gender)

		if face_selector_race in nthx_fusion.choices.races:
