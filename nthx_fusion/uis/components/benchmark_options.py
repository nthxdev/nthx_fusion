...
from nthx_fusion.types import BenchmarkMode, BenchmarkResolution
...
def render() -> None:
	...
	BENCHMARK_MODE_DROPDOWN = gradio.Dropdown(
		...
		choices = nthx_fusion.choices.benchmark_modes,
	...
	BENCHMARK_RESOLUTIONS_CHECKBOX_GROUP = gradio.CheckboxGroup(
		...
		choices = nthx_fusion.choices.benchmark_resolutions,
	...
	BENCHMARK_CYCLE_COUNT_SLIDER = gradio.Slider(
		...
		step = calculate_int_step(nthx_fusion.choices.benchmark_cycle_count_range),
		minimum = nthx_fusion.choices.benchmark_cycle_count_range[0],
		maximum = nthx_fusion.choices.benchmark_cycle_count_range[-1]
	)
