...
from nthx_fusion import face_landmarker, state_manager, translator
from nthx_fusion.common_helper import calculate_float_step
from nthx_fusion.types import FaceLandmarkerModel, Score
...
def render() -> None:
	...
	FACE_LANDMARKER_MODEL_DROPDOWN = gradio.Dropdown(
		...
		choices = nthx_fusion.choices.face_landmarker_models,
	...
	FACE_LANDMARKER_SCORE_SLIDER = gradio.Slider(
		...
		step = calculate_float_step(nthx_fusion.choices.face_landmarker_score_range),
		minimum = nthx_fusion.choices.face_landmarker_score_range[0],
		maximum = nthx_fusion.choices.face_landmarker_score_range[-1]
	)
