...
from nthx_fusion import face_masker, state_manager, translator
from nthx_fusion.common_helper import calculate_float_step, calculate_int_step
from nthx_fusion.sanitizer import sanitize_int_range
from nthx_fusion.types import FaceMaskArea, FaceMaskRegion, FaceMaskType, FaceOccluderModel, FaceParserModel
...
def render() -> None:
	...
	with gradio.Row():
		FACE_OCCLUDER_MODEL_DROPDOWN = gradio.Dropdown(
			...
			choices = nthx_fusion.choices.face_occluder_models,
		...
		FACE_PARSER_MODEL_DROPDOWN = gradio.Dropdown(
			...
			choices = nthx_fusion.choices.face_parser_models,
	...
	FACE_MASK_TYPES_CHECKBOX_GROUP = gradio.CheckboxGroup(
		...
		choices = nthx_fusion.choices.face_mask_types,
	...
	FACE_MASK_AREAS_CHECKBOX_GROUP = gradio.CheckboxGroup(
		...
		choices = nthx_fusion.choices.face_mask_areas,
	...
	FACE_MASK_REGIONS_CHECKBOX_GROUP = gradio.CheckboxGroup(
		...
		choices = nthx_fusion.choices.face_mask_regions,
	...
	FACE_MASK_BLUR_SLIDER = gradio.Slider(
		...
		step = calculate_float_step(nthx_fusion.choices.face_mask_blur_range),
		minimum = nthx_fusion.choices.face_mask_blur_range[0],
		maximum = nthx_fusion.choices.face_mask_blur_range[-1],
	...
	with gradio.Group(visible = has_box_mask) as FACE_MASK_BOX_WRAPPER:
		with gradio.Row():
			FACE_MASK_PADDING_TOP_SLIDER = gradio.Slider(
				...
				step = calculate_int_step(nthx_fusion.choices.face_mask_padding_range),
				minimum = nthx_fusion.choices.face_mask_padding_range[0],
				maximum = nthx_fusion.choices.face_mask_padding_range[-1],
...
def update_face_mask_types(face_mask_types : List[FaceMaskType]) -> Tuple[gradio.CheckboxGroup, gradio.CheckboxGroup, gradio.CheckboxGroup, gradio.Slider, gradio.Group]:
	face_mask_types = face_mask_types or nthx_fusion.choices.face_mask_types
...
def update_face_mask_areas(face_mask_areas : List[FaceMaskArea]) -> gradio.CheckboxGroup:
	face_mask_areas = face_mask_areas or nthx_fusion.choices.face_mask_areas
...
def update_face_mask_regions(face_mask_regions : List[FaceMaskRegion]) -> gradio.CheckboxGroup:
	face_mask_regions = face_mask_regions or nthx_fusion.choices.face_mask_regions
...
def update_face_mask_padding(face_mask_padding_top : float, face_mask_padding_right : float, face_mask_padding_bottom : float, face_mask_padding_left : float) -> None:
	face_mask_padding_top = sanitize_int_range(int(face_mask_padding_top), nthx_fusion.choices.face_mask_padding_range)
	face_mask_padding_right = sanitize_int_range(int(face_mask_padding_right), nthx_fusion.choices.face_mask_padding_range)
	face_mask_padding_bottom = sanitize_int_range(int(face_mask_padding_bottom), nthx_fusion.choices.face_mask_padding_range)
	face_mask_padding_left = sanitize_int_range(int(face_mask_padding_left), nthx_fusion.choices.face_mask_padding_range)
