...
from nthx_fusion import ffmpeg_builder, logger, state_manager, translator
from nthx_fusion.audio import create_empty_audio_frame
from nthx_fusion.content_analyser import analyse_stream
from nthx_fusion.ffmpeg import open_ffmpeg
from nthx_fusion.filesystem import is_directory
from nthx_fusion.processors.core import get_processors_modules
from nthx_fusion.types import Fps, StreamMode, VisionFrame
from nthx_fusion.vision import extract_vision_mask, read_static_images
	...
	with tqdm(desc = translator.get('streaming'), unit = 'frame', disable = state_manager.get_item('log_level') in [ 'warn', 'error' ]) as progress:
		with ThreadPoolExecutor(max_workers = state_manager.get_item('execution_thread_count')) as executor:
			...
			while camera_capture and camera_capture.isOpened():
				...
				if not state_manager.get_item('skip_content_analysis') and analyse_stream(capture_vision_frame, camera_fps):
