...
from nthx_fusion.types import LogLevel


def init(log_level : LogLevel) -> None:
	basicConfig(format = '%(message)s')
	get_package_logger().setLevel(nthx_fusion.choices.log_level_set.get(log_level))


def get_package_logger() -> Logger:
	return getLogger('nthx_fusion')
...
def create_message(message : str, module_name : str) -> str:
	...
	if first_module_name and last_module_name:
		brand_name = 'NTHX_FUSION' if first_module_name == 'nthx_fusion' else first_module_name.upper()
		return '[' + brand_name + '.' + last_module_name.upper() + '] ' + message
	return message
