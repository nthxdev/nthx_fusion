...
import nthx_fusion.jobs.job_manager
import nthx_fusion.jobs.job_store
from nthx_fusion import config, face_detector, face_landmarker, face_recognizer, inference_manager, logger, state_manager, translator, video_manager
from nthx_fusion.common_helper import get_first, get_middle, is_macos
from nthx_fusion.download import conditional_download_hashes, conditional_download_sources, resolve_download_url
from nthx_fusion.execution import has_execution_provider
from nthx_fusion.face_creator import average_face_identity, get_one_face, get_static_faces, scale_face
from nthx_fusion.face_helper import paste_back, warp_face_by_face_landmark_5
from nthx_fusion.face_masker import create_area_mask, create_box_mask, create_occlusion_mask, create_region_mask
from nthx_fusion.face_selector import select_faces, sort_faces_by_order
from nthx_fusion.filesystem import filter_image_paths, has_image, in_directory, is_image, is_video, resolve_relative_path, same_file_extension
from nthx_fusion.model_helper import get_static_model_initializer
from nthx_fusion.processors.modules.face_swapper import choices as face_swapper_choices
from nthx_fusion.processors.modules.face_swapper.types import FaceSwapperInputs
from nthx_fusion.processors.pixel_boost import explode_pixel_boost, implode_pixel_boost
from nthx_fusion.processors.types import ProcessorOutputs
from nthx_fusion.program_helper import find_argument_group
from nthx_fusion.thread_helper import conditional_thread_semaphore
from nthx_fusion.types import ApplyStateItem, Args, DownloadScope, Embedding, Face, InferencePool, InferenceProvider, ModelOptions, ModelSet, ProcessMode, VisionFrame
from nthx_fusion.vision import read_static_image, read_static_images, read_static_video_chunk, read_static_video_frame, unpack_resolution
...
def resolve_inference_providers() -> List[InferenceProvider]:
	...
	if is_macos() and has_execution_provider('coreml'):
		if model_type in [ 'ghost', 'uniface' ] or model_precision == 'fp16':
			return\
			[
				(nthx_fusion.choices.execution_provider_set.get('coreml'),
				{
...
def register_args(program : ArgumentParser) -> None:
	...
	if group_processors:
		...
		nthx_fusion.jobs.job_store.register_step_keys([ 'face_swapper_model', 'face_swapper_pixel_boost', 'face_swapper_weight' ])
...
def get_common_modules() -> List[ModuleType]:
	return [ face_detector, face_landmarker, face_recognizer ]
