...
from nthx_fusion import config, content_analyser, inference_manager, logger, state_manager, translator, video_manager
from nthx_fusion.common_helper import is_macos, is_windows
...
from nthx_fusion.normalizer import normalize_color
from nthx_fusion.processors.modules.background_remover import choices as background_remover_choices
from nthx_fusion.processors.modules.background_remover.types import BackgroundRemoverInputs
...
from nthx_fusion.types import ApplyStateItem, Args, DownloadScope, InferencePool, InferenceProvider, Mask, ModelOptions, ModelSet, ProcessMode, VisionFrame
...
def resolve_inference_providers() -> List[InferenceProvider]:
	...
	if is_macos() and has_execution_provider('coreml') or is_windows() and has_execution_provider('directml') and model_type == 'corridor_key':
		return [ nthx_fusion.choices.execution_provider_set.get('cpu') ]
...
def register_args(program : ArgumentParser) -> None:
	...
	if group_processors:
		...
		nthx_fusion.jobs.job_store.register_step_keys([ 'background_remover_model', 'background_remover_fill_color', 'background_remover_despill_color' ])
